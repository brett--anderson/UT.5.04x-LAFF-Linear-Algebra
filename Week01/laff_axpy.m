...
% y = axpy( x, y ) copies vector x times alpha + y into vector y
...
if ( n_x == 1 )     % x is a column vector
    if ( n_y == 1 )     % y is a column vector
        % Copy the elements of  x times alpha + y into the elements of y
        for i=1:m_x   
    ...
    if ( n_y == 1 )     % y is a column vector
        % Copy the elements of  x times alpha + y into the elements of y
        for i=1:n_x   
            y( i,1 ) = x( 1,i ) * alpha + y( i,1 );
        end
    else     % y is a row vector
        % Copy the elements of x times alpha + y into the elements of y
        for i=1:n_x   
            y( 1,i ) = x( 1,i ) * alpha + y( 1,i );
        end
    end
end
